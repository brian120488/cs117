...
main(int argc, char *argv[]) {
    ...
    //        Send / receive / print 
    //
    char input[512]; // TODO: what abt bigger inputs?
    while (true) {
        cout << "Message: ";
        ...
        try {
            ...
            bool retry = true;
            while (tries-- > 0 && retry) {
                cout << "Reading attempt " << 5 - tries << "..." << endl;

                // Send the message to the server
                c150debug->printf(C150APPLICATION,"%s: Writing message: \"%s\"",
                                argv[0], input);
                sock -> write(input, strlen(input)+1); // +1 includes the null
            ...
            // TODO: dont seem to need this? checking for more packets
            // ssize_t old_readlen = readlen;
            // while (old_readlen < (readlen = sock->read(incomingMessage, sizeof(incomingMessage)))) {
        ...
        //
            ...
            cerr << argv[0] << ": caught C150NetworkException: " << "server no respond!"\
                            << endl;
        }
    }
